...
public class CompatibleWorldRenderer extends EntityRenderer implements IResourceManagerReloadListener {
		...
		if (OpenGlHelper.shadersSupported) {
			...
			if (this.shaderIndex != shaderCount) {
				...
				try {
					...
					logger.warn("Failed to load shader: " + shaderResourceLocations[this.shaderIndex],
	...
	public void updateRenderer() {
		...
		float f = this.mc.theWorld.getLightBrightness(MathHelper.floor_double(this.mc.renderViewEntity.posX),
	...
	public void getMouseOver(float p_78473_1_) {
		if (this.mc.renderViewEntity != null) {
			if (this.mc.theWorld != null) {
				...
				Vec3 vec33 = null;
				float f1 = 1.0F;
				List<Entity> list = this.mc.theWorld.getEntitiesWithinAABBExcludingEntity(
						(Entity) this.mc.renderViewEntity, this.mc.renderViewEntity.boundingBox
	...
	private void setupViewBobbing(float p_78475_1_) {
		if (this.mc.renderViewEntity instanceof EntityPlayer) {
			...
			GL11.glTranslatef(MathHelper.sin(f2 * 3.1415927F) * f3 * 0.5F,
					-Math.abs(MathHelper.cos(f2 * 3.1415927F) * f3), 0.0F);
	...
	private void orientCamera(float p_78467_1_) {
		...
		if (entitylivingbase.isPlayerSleeping()) {
			...
			if (this.mc.gameSettings.debugCamEnable) {
				...
				double d3 = (-MathHelper.sin(f6 / 180.0F * 3.1415927F) * MathHelper.cos(f2 / 180.0F * 3.1415927F)) * d7;
				double d4 = (MathHelper.cos(f6 / 180.0F * 3.1415927F) * MathHelper.cos(f2 / 180.0F * 3.1415927F)) * d7;
				double d5 = -MathHelper.sin(f2 / 180.0F * 3.1415927F) * d7;
		...
		if(!ClientHandler.onCameraUpdate(entitylivingbase,p_78467_1_))
			if (!this.mc.gameSettings.debugCamEnable) {
		...
		f1 = Interceptors.adjustCameraPosition(entitylivingbase, f1);
	...
	private void setupCameraTransform(float p_78479_1_, int p_78479_2_) {
		...
		Project.gluPerspective(getFOVModifier(p_78479_1_, true), this.mc.displayWidth / this.mc.displayHeight, 0.05F,
		...
		float f2 = this.mc.thePlayer.prevTimeInPortal
	...
	private void renderHand(float p_78476_1_, int p_78476_2_) {
		if (this.debugViewDirection <= 0) {
			...
			Project.gluPerspective(getFOVModifier(p_78476_1_, false), this.mc.displayWidth / this.mc.displayHeight,
		...
		if (worldclient != null) {
			...
			for (int i = 0; i < 256; i++) {
				...
				if (this.bossColorModifier > 0.0F) {
					...
					float f = this.bossColorModifierPrev
							+ (this.bossColorModifier - this.bossColorModifierPrev) * p_78472_1_;
					f8 = f8 * (1.0F - f) + f8 * 0.7F * f;
					f9 = f9 * (1.0F - f) + f9 * 0.6F * f;
					f10 = f10 * (1.0F - f) + f10 * 0.6F * f;
				}
				...
				if (this.mc.thePlayer.isPotionActive(Potion.nightVision)) {
					...
					float f15 = getNightVisionBrightness((EntityPlayer) this.mc.thePlayer, p_78472_1_);
					float f16 = 1.0F / f8;
					...
					f8 = f8 * (1.0F - f15) + f8 * f16 * f15;
					f9 = f9 * (1.0F - f15) + f9 * f16 * f15;
					f10 = f10 * (1.0F - f15) + f10 * f16 * f15;
				}
	...
	public void updateCameraAndRender(float p_78480_1_) {
		...
		if (this.mc.inGameHasFocus && flag) {
			...
			if (this.mc.gameSettings.smoothCamera) {
				...
				this.mc.thePlayer.setAngles(this.smoothCamFilterX * f5, this.smoothCamFilterY * f5 * b0);
			} else {
		...
		if (!this.mc.skipRenderWorld) {
			...
			if (this.mc.currentScreen != null) {
				...
				try {
					...
					crashreportcategory.addCrashSectionCallable("Screen name", new Callable() { // from class:
																								// com.vicmatskiv.weaponlib.compatibility.CompatibleWorldRenderer.1
					...
					crashreportcategory.addCrashSectionCallable("Mouse location", new Callable() { // from class:
																									// com.vicmatskiv.weaponlib.compatibility.CompatibleWorldRenderer.2
						private static final String __OBFID = "CL_00000950";

						@Override // java.util.concurrent.Callable
						public String call() {
							return String.format("Scaled: (%d, %d). Absolute: (%d, %d)", Integer.valueOf(k),
									Integer.valueOf(l), Integer.valueOf(Mouse.getX()), Integer.valueOf(Mouse.getY()));
						}
					});
					crashreportcategory.addCrashSectionCallable("Screen size", new Callable() { // from class:
																								// com.vicmatskiv.weaponlib.compatibility.CompatibleWorldRenderer.3
						...
						public String call() {
							return String.format("Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %d",
									...
									Integer.valueOf(CompatibleWorldRenderer.this.mc.displayWidth),
									Integer.valueOf(CompatibleWorldRenderer.this.mc.displayHeight),
	...
	public void renderWorld(float p_78471_1_, long p_78471_2_) {
		renderWorldWithoutShaders(p_78471_1_, p_78471_2_);
		if (OpenGlHelper.shadersSupported) {
			...
			if (this.theShaderGroup != null && this.useShader) {
				...
				GL11.glPushMatrix();
				GL11.glLoadIdentity();
				this.theShaderGroup.loadShaderGroup(p_78471_1_);
				GL11.glPopMatrix();
			}
	...
	public void renderWorldWithoutShaders(float p_78471_1_, long p_78471_2_) {
		this.mc.mcProfiler.startSection("lightTex");
		...
		for (int j = 0; j < 2; j++) {
			...
			ActiveRenderInfo.updateRenderInfo((EntityPlayer) this.mc.thePlayer,
			...
			if (this.debugViewDirection == 0) {
				...
				if (this.mc.objectMouseOver != null && entitylivingbase.isInsideOfMaterial(Material.water)
						&& entitylivingbase instanceof EntityPlayer && !this.mc.gameSettings.hideGUI) {

					EntityPlayer entityplayer = (EntityPlayer) entitylivingbase;
					GL11.glDisable(3008);
					this.mc.mcProfiler.endStartSection("outline");
					if (!ForgeHooksClient.onDrawBlockHighlight(renderglobal, entityplayer, this.mc.objectMouseOver, 0,
			...
			if (this.cameraZoom == 1.0D && entitylivingbase instanceof EntityPlayer && !this.mc.gameSettings.hideGUI
					&& this.mc.objectMouseOver != null && !entitylivingbase.isInsideOfMaterial(Material.water)) {
			...
			for (int j1 = 0; j1 < i1; j1++) {
				...
				if (i2 <= j + b0 && i2 >= j - b0 && biomegenbase.canSpawnLightningBolt()
					...
					if (block.getMaterial() == Material.lava) {
						...
						this.mc.effectRenderer.addEffect((EntityFX) new EntitySmokeFX((World) worldclient, (k1 + f1),
						...
						this.mc.effectRenderer.addEffect((EntityFX) new EntityRainFX((World) worldclient, (k1 + f1),
					...
					if (biomegenbase.canSpawnLightningBolt() || biomegenbase.getEnableSnow()) {
						...
						if (l1 != i2) {
							...
							if (worldclient.getWorldChunkManager().getTemperatureAtHeight(f9, k1) >= 0.15F) {
								...
								float f10 = ((this.rendererUpdateCount + i1 * i1 * 3121 + i1 * 45238971 + l * l * 418711
										+ l * 13761 & 0x1F) + p_78474_1_) / 32.0F * (3.0F + this.random.nextFloat());
	...
	private void updateFogColor(float p_78466_1_) {
		...
		Vec3 vec3 = worldclient.getSkyColor((Entity) this.mc.renderViewEntity, p_78466_1_);
		...
		if (this.mc.gameSettings.renderDistanceChunks >= 4) {
			...
			Vec3 vec32 = (MathHelper.sin(worldclient.getCelestialAngleRadians(p_78466_1_)) > 0.0F)
					...
					: Vec3.createVectorHelper(1.0D, 0.0D, 0.0D);
			float f = (float) entitylivingbase.getLook(p_78466_1_).dotProduct(vec32);
			...
			if (f > 0.0F) {
				...
				if (afloat != null) {
					...
					f *= afloat[3];
					this.fogColorRed = this.fogColorRed * (1.0F - f) + afloat[0] * f;
					this.fogColorGreen = this.fogColorGreen * (1.0F - f) + afloat[1] * f;
					this.fogColorBlue = this.fogColorBlue * (1.0F - f) + afloat[2] * f;
				}
			}
		}
		...
		Block block = ActiveRenderInfo.getBlockAtEntityViewpoint((World) this.mc.theWorld, entitylivingbase,
				p_78466_1_);
		...
		if (this.cloudFog) {
			...
			float f = EnchantmentHelper.getRespiration(entitylivingbase) * 0.2F;
			this.fogColorRed = 0.02F + f;
			this.fogColorGreen = 0.02F + f;
			this.fogColorBlue = 0.2F + f;
		...
		if (entitylivingbase.isPotionActive(Potion.nightVision)) {
			...
			float f11 = getNightVisionBrightness((EntityPlayer) this.mc.thePlayer, p_78466_1_);
		...
		EntityViewRenderEvent.FogColors event = new EntityViewRenderEvent.FogColors(this, entitylivingbase, block,
				p_78466_1_, this.fogColorRed, this.fogColorGreen, this.fogColorBlue);
	...
	private void setupFog(int p_78468_1_, float p_78468_2_) {
		...
		if (p_78468_1_ == 999) {
			...
			EntityViewRenderEvent.FogDensity event = new EntityViewRenderEvent.FogDensity(this, entitylivingbase, block,
					p_78468_2_, 0.1F);
			...
			if (MinecraftForge.EVENT_BUS.post((Event) event)) {
				...
				if (this.mc.theWorld.provider.getWorldHasVoidParticles() && !flag) {
					...
					double d0 = ((entitylivingbase.getBrightnessForRender(p_78468_2_) & 0xF00000) >> 20) / 16.0D
				...
				MinecraftForge.EVENT_BUS.post((Event) new EntityViewRenderEvent.RenderFogEvent(this, entitylivingbase,
						block, p_78468_2_, p_78468_1_, f1));
			}
	...
	public void setShaderGroup(DynamicShaderGroup shaderGroup) {
		this.theShaderGroup = (ShaderGroup) shaderGroup;
	}
...
/*
 * Location: I:\BON\mwmodify-deobf.jar!\com\vicmatskiv\weaponlib\compatibility\
 * CompatibleWorldRenderer.class Java compiler version: 8 (52.0) JD-Core
 * Version: 1.1.3
 */